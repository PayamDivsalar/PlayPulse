...
- `scripts/bring_up.sh` — prepare env files, start the Compose stack, wait for health
...
| `deploy`        | Run `scripts/bring_up.sh --skip-check`, then `scripts/infra/check_infra.sh` to confirm health |
...
Full deployment (installs Docker, clones repo, renders `.env`, deploys, health-checks):
...
# (git pull + re-render .env + bring_up.sh + check_infra.sh)
...
| `deploy`        | git clone/pull + `.env` render + `bring_up.sh` + `check_infra.sh` |
...
- Idempotency: on a second full run the `docker` and `project_setup` roles report
  no changes. The `deploy` role executes shell scripts, so Ansible always marks
